...
resource "segment_destination_subscription" "id-6648883872e824ad8803c1a9_sg7AbX42yMfqUFS8APkQHn" {
  ...
  name           = "Send Track"
  settings       = "{\"anonymous_id\":{\"@path\":\"$.properties.ANONYMOUS_ID\"},\"consent\":{\"@path\":\"$.context.consent\"},\"enable_batching\":false,\"event_name\":{\"@path\":\"$.properties.EVENT_TEXT\"},\"message_id\":{\"@path\":\"$.messageId\"},\"properties\":{\"@path\":\"$.properties\"},\"timestamp\":{\"@path\":\"$.properties.ORIGINAL_TIMESTAMP\"},\"user_id\":{\"@path\":\"$.properties.USER_ID\"}}"
  trigger        = "event = \"new\""
}